...
import com.fuzora.external.impl.Action;
...
import com.fuzora.transformer.runner.TransformerConfig;
...
public class ConfigReader implements ApplicationContextAware {
	...
	public ConfigReader(PipelineRunner pipeline) {
	...
	public void readConfigFiles() throws IOException {
		...
		JsonNode configFile = objectMapper.readTree(jsonResource.getInputStream());
	...
	private void setupPipeline() {
		...
		DataHandler actionHandler = (DataHandler) this.applicationContext.getBean("rabbitOutput");
		pipeline.setup(filterHandler, transformHandler, actionHandler);

	}

	private Map<String, Object> deployTriggerConfig() {
		return switch (this.triggerProtocol) {
	...
	private void deployTransformerConfig() {
		this.applicationContext.getBean(TransformerConfig.class).apply(this.transformerConfig);

	}

	private Map<String, Object> deployActionConfig() {
		return switch (this.actionProtocol) {
